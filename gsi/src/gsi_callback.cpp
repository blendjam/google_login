...
static gsi_callback            m_callback;
...
static void gsi_invoke_callback(MESSAGE_ID type, const char* json)
{
    gsi_callback* cbk = &m_callback;
    if (cbk->m_Callback == LUA_NOREF)
    {
        dmLogInfo("GSI callback do not exist.");
        return;
    }
    ...
    {
        UnregisterCallback();
        dmLogError("Could not run GSI callback because the instance has been deleted.");
        lua_pop(L, 2);
    }
    else {
...
void gsi_callback_update()
{
    ...
    for (uint32_t i = 0; i != tmp.Size(); ++i)
    {
        ...
        gsi_invoke_callback(data->msg, data->json);
        if (data->json)
        {
            free(data->json);
            data->json = 0;
